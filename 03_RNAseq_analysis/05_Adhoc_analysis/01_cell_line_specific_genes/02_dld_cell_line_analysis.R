...
dld_line = extract_cell_line_count(input_file = file_paths, cell_line_name = "dld", write_output = FALSE)

# Save the output
write.table(dld_line, file = file.path(store_dir, "gene_expression_dld_cell_line.tsv"), row.names = FALSE, sep = "\t")

# Get the count matrix
temp_counts = as.data.frame(dld_line$counts)
...
log2(temp_counts["ENSG00000165819", "dld_ctr"]/temp_counts["ENSG00000165819", "dld_ki"]) # 0.219678 log2
...
temp_counts$log2fc = log2(temp_counts$dld_ctr/temp_counts$dld_ki)
