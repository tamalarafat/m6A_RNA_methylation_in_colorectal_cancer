...
crtl_degs = read.table("~/Documents/Projects/Git_repositories/effect_of_METTL3_on_m6A_in_cancer/Analyses/Analysis_of_all_samples/01_whole_data_analysis/02_analysis_on_counts/analysis_output/06_characteristic_degs_lowFC/DEG_up_in_ctr_group.csv")
...
ctrl_df = data.frame("up" = c(3381, 223), "nonUp" = c(7259, 866), row.names = c("DRACH", "DEGs")) # 223 out of 1089

fisher.test(ctrl_df) # p-value = 2.148e-15

fileGenerator(temp_ctrl_common, fileName = "DRACH_DEG_CTRL_Common.txt")
...
# load the degs identified in the control group
ki_degs = read.table("~/Documents/Projects/Git_repositories/effect_of_METTL3_on_m6A_in_cancer/Analyses/Analysis_of_all_samples/01_whole_data_analysis/02_analysis_on_counts/analysis_output/06_characteristic_degs_lowFC/DEG_up_in_ki_group.csv")
...
ki_df = data.frame("up" = c(806, 54), "nonUp" = c(9871, 998), row.names = c("DRACH", "DEGs")) # 54 out of 1052

fisher.test(ki_df) # p-value = 0.003502

fileGenerator(temp_ki_common, fileName = "DRACH_DEG_ki_Common.txt")
