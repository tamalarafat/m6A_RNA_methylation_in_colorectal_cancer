...
ctrl_deg = read.table("~/Documents/Projects/Git_repositories/effect_of_METTL3_on_m6A_in_cancer/Analyses/Analysis_of_all_samples/01_whole_data_analysis/02_analysis_on_counts/analysis_output/06_characteristic_degs_lowFC/DEG_up_in_ctr_group.csv")
...
ggsave(filename = "06_Figure_5_FC.png", plot = p, width = 8, height = 8, dpi = 300)


# Alternative

# Define the base plot with main dataset
p <- ggplot(df, aes(x = log2FC, y = point_size, color = group)) + 
  # Main points for all data excluding subset
  geom_point(data = df[-which(df$ID %in% temp_DRACH_genes), ], 
             aes(size = point_size), 
             alpha = 0.6) +
  
  # Points for the special subset (top_points)
  geom_point(data = top_points, 
             aes(size = point_size), 
             color = "#00A08A", 
             alpha = 1.5) +
  
  # Repelling text labels for top points
  ...
  scale_color_manual(values = grp_col, 
                     labels = c("control" = "Control", "ki" = "Knock-down")) +
  
  # Customizing the legend
  guides(
    color = guide_legend(title = "Conditions", override.aes = list(size = 4)),
    size = "none"
  )
p
