...
temp_df$hgnc_symbol = genes_table$hgnc_symbol[match(temp_df$ref_seq_id, genes_table$ensembl_transcript_id_version)]

temp_df$entrezgene_id = genes_table$entrezgene_id[match(temp_df$ref_seq_id, genes_table$ensembl_transcript_id_version)]
...
p <- ggplot(df, aes(x = diff_mod_rate, y = point_size, color = diff_mod)) + 
  ...
  geom_text_repel(data = top_points, aes(label = hgnc_symbol), nudge_x = -15, size = 5, force = 5) + 
...
ggsave(filename = "04_Figure_3.png", plot = p, width = 8, height = 8, dpi = 300)
